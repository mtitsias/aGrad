...
% compute statistics 
summaryAuxZ{ds} = summaryStatistics(samples);
summaryAuxZ{ds}.elapsed = elapsedTime;
summaryAuxZ{ds}.accRates = accRates;
summaryAuxZ{ds}.delta = model.delta; 
summaryAuxZ{ds}.eff_LogL = mcmc_ess(samples.LogL(mcmcoptions.Burnin+1:end));
