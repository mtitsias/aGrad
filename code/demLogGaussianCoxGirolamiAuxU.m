function demLogGaussianCoxGirolamiAuxU(rep)

addpath toolbox/; 
 
dataName = 'logGaussianCoxGirolami_AuxU';
storeRes = 1;
...
% compute statistics 
summaryAuxU{ds} = summaryStatistics(samples);
summaryAuxU{ds}.elapsed = elapsedTime;
summaryAuxU{ds}.accRates = accRates;
summaryAuxU{ds}.delta = model.delta; 
summaryAuxU{ds}.eff_LogL = mcmc_ess(samples.LogL(mcmcoptions.Burnin+1:end));
...
if storeRes == 1
    ...
    if rep == 1
       save(['../results/' dataName '_repeat' num2str(rep) '.mat'], 'summaryAuxU', 'model', 'samples');
    else
