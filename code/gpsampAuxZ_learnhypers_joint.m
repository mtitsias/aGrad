function [model samples accRates] = gpsampAuxZ_learnhypers_joint(model, trainOps)
%
% it samples also hyperparameters with joint sampling of the latent field and theta
...
for j=1:model.J
   ...
   if strcmp(model.constraints.kernHyper, 'free')
      % a marginal likelihood appears in the acceptance ratio
      model.GP{j}.LogDetK = 2*sum(log(diag(margL)));
      model.GP{j}.invL = marginvL';
   end  
end 
...
%
    ...
    if Langevin == 1
       for j=1:model.J  
          Z(j,:) = F(j,:) + (model.delta/2)*(derF(:,j)') + sqrt(model.delta/2)*randn(1,n);
       end
    else
       for j=1:model.J
          Z(j,:) = F(j,:) +sqrt(model.delta/2).*randn(1,n);
       end
    end
     
    % STEP 2: M-H step
    for j=1:model.J  
       Fnew(j,:) = (randn(1, n) + ((2/model.delta)*Z(j,:))*model.GP{j}.auxPostL')*model.GP{j}.auxPostL;
       %cmu = ((2/model.delta)*Z(j,:))*model.GP{j}.invK;
       %Fnew(j,:) = gaussianFastSample(1, cmu, model.GP{j}.auxPostL);
    end
    ...
    if Langevin == 1 
        ...
        % correction term
        gzy = sum(sum( (Z - Fnew - (model.delta/4)*derFnew').*(derFnew') ));
        gzx = sum(sum( (Z - F - (model.delta/4)*derF').*(derF') ));
        corrFactor = gzy - gzx;
    end
    [accept, uprob] = metropolisHastings(newLogLik+corrFactor, oldLogLik, 0, 0); 
    %newLogLik + corrFactor - oldLogLik
    ...
    % sample kernel hyperparameters jointly with the field
    if strcmp(model.constraints.kernHyper, 'free')
    %     
       GPnew = model.GP;  
       oldlogGP = zeros(1,model.J); % this stores the auxiliary marginal likelihood and not the GP prior!
       newlogGP = zeros(1,model.J); % this stores the auxiliary marginal likelihood and not the GP prior!
       newLogPriorK = zeros(1,model.J); 
       % sample every (field_j, hypers_j) joinlty and sequentially, i.e. conditional on the rest fields 
       for j=1:model.J
          Fnew = F; 
          % resample all auxiliary variables  
          if Langevin == 1  
              Z = F + (model.delta/2)*(derF') + sqrt(model.delta/2)*randn(model.J, n);
          else
              Z = F + sqrt(model.delta/2).*randn(model.J, n);
          end   
          
          % propose new hyperparameter for j-th GP 
          ...
          newL = chol(GPnew{j}.K + (model.delta/2)*eye(model.n));  
          tmp = newL\eye(n);
          marginvL = tmp;
          tmp = tmp*(model.delta/2);
          GPnew{j}.invK = (model.delta/2)*eye(n) - tmp*tmp';
          GPnew{j}.auxPostL = jitterChol(GPnew{j}.invK);
             
          % given the proposed hypers proposed a new field for the j-th GP  
          Fnew(j,:) = (randn(1, n) + ((2/model.delta)*Z(j,:))*GPnew{j}.auxPostL')*GPnew{j}.auxPostL;
              
          % evaluate the log GP marginal likelihood, Z(theta) and
          % Z(theta'), that you need in the M-H ratio
          GPnew{j}.invL = marginvL';
          ...
          if Langevin == 1         
             ...
             %% correction for the z auxiliary scheme 
             gzy = sum(sum( (Z - Fnew - (model.delta/4)*derFnew').*(derFnew') ));
             gzx = sum(sum( (Z - F - (model.delta/4)*derF').*(derF') ));        
             corrFactor = gzy - gzx;
          end
          ...
          % Metropolis-Hastings to accept-reject the proposal
          %accept = metropolisHastings(newLogLik + corrFactor + newlogGP, oldLogLik + oldlogGP, 0, 0);
          accept = metropolisHastings(newLogLik + corrFactor + newlogGP(j) + newLogPriorK(j), oldLogLik + oldlogGP(j) + oldLogPriorK(j), 0, 0);
          %
          if accept == 1
          % 
             model.GP{j} = GPnew{j};
    ...
    if mod(it,5) == 0 
    % Adapt the GP function proposal, i.e. the step size delta during burnin 
    if (it >= 50) 
