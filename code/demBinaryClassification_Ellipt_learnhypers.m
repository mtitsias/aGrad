...
%     
   ...
   tic;
   [model samples] = gpsampEllipt_learnhypers(model, mcmcoptions);
   elapsedTime = toc;
   ...
   for j=1:model.GP{1}.nParams
      summaryEllipt.eff_kern(j) = mcmc_ess(samples.kernLogtheta{1}(:,j));
   end
   
   save(['../results/' dataName{1} '_Ellipt_learnhypers.mat'], 'summaryEllipt', 'model', 'samples');
%    
end