...
%     
   ...
   % normalize the data so tha the inputs have unit variance and zero mean  
   meanX = mean(X);
   sqrtvarX = sqrt(var(X)); 
   ...
   tic;
   [model samples accRates] = gpsampAuxZ_learnhypers_joint(model, mcmcoptions);
   elapsedTime = toc;

   % compute statistics  
   samples.F = squeeze(samples.F)';
   summaryAuxZadvanced = summaryStatistics(samples);
   summaryAuxZadvanced.elapsed = elapsedTime;
   summaryAuxZadvanced.accRates = accRates;
   summaryAuxZadvanced.delta = model.delta; 
   summaryAuxZadvanced.eff_LogL = mcmc_ess(samples.LogL(mcmcoptions.Burnin+1:end));
   summaryAuxZadvanced.kerndelta = model.kerndelta;
   for j=1:model.GP{1}.nParams 
      summaryAuxZadvanced.eff_kern(j) = mcmc_ess(samples.kernLogtheta{1}(:,j));
   end
 
   save(['../results/' dataName{1} '_AuxZadvanced_learnhypers.mat'], 'summaryAuxZadvanced', 'model', 'samples');
%    
end