...
%     
   ...
   tic;
   [model samples accRates] = gpsampAuxZ_learnhypers(model, mcmcoptions);
   elapsedTime = toc;

   % compute statistics  
   samples.F = squeeze(samples.F)';
   summaryAuxZ = summaryStatistics(samples);
   summaryAuxZ.elapsed = elapsedTime;
   summaryAuxZ.accRates = accRates;
   summaryAuxZ.delta = model.delta; 
   summaryAuxZ.eff_LogL = mcmc_ess(samples.LogL(mcmcoptions.Burnin+1:end));
   summaryAuxZ.kerndelta = model.kerndelta;
   for j=1:model.GP{1}.nParams 
      summaryAuxZ.eff_kern(j) = mcmc_ess(samples.kernLogtheta{1}(:,j));
   end
 
   save(['../results/' dataName{1} '_AuxZ_learnhypers.mat'], 'summaryAuxZ', 'model', 'samples');
%    
end