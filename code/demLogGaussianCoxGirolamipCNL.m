function demLogGaussianCoxGirolamipCNL(rep)

addpath toolbox/; 
dataName = 'logGaussianCoxGirolami_pCNL';
storeRes = 1;
...
summarypCNL{ds}.delta = model.delta; 
summarypCNL{ds}.beta = model.beta; 
summarypCNL{ds}.eff_LogL = mcmc_ess(samples.LogL(mcmcoptions.Burnin+1:end));
...
if storeRes == 1 
    ...
    if rep == 1
       save(['../results/' dataName '_repeat' num2str(rep) '.mat'], 'summarypCNL', 'model', 'samples');
    else
